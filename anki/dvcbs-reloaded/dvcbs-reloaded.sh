...
function checktype()
{
    ...
    elif [ ${BUILD_TYPE} == "prodcloudless" ]; then
        BUILD_SUFFIX=cldless
        echo "Prod Cloudless build type selected. This build will be signed and installable from recovery."
    elif [ ${BUILD_TYPE} == "oskrcloudless" ]; then
        BUILD_SUFFIX=oskrcldless
        echo "OSKR Cloudless build type selected. This build won't be signed."
    elif [ ${BUILD_TYPE} == "devcloudless" ]; then
        BUILD_SUFFIX=dcldless
        echo "Dev Cloudless build type selected. Note that this won't work on your OSKR bot. Only unlocked bots. This build won't be signed."
else
...
function mountota()
{
    ...
    if [ -d ${dir}apps_proc ]; then
        ...
        pigz -d ${dir}apq8009-robot-sysfs.img.gz
    ...
    pigz -d ${dir}apq8009-robot-sysfs.img.dec.gz
...
function buildcustomandsign()
{
    ...
    pigz --best -k ${dir}apq8009-robot-sysfs.img
    mkdir ${dir}final
    echo "Encrypting"
    ...
    pigz -d ${dir}tempSign/apq8009-robot-sysfs.img.dec.gz
    ...
    pigz -d ${refo}/tempBoot/apq8009-robot-boot.img.dec.gz
    ...
    if [[ ${BUILD_TYPE} == "prod" || ${BUILD_TYPE} == "prodcloudless" ]]; then
